...
\newcommand{\liso}{L_{iso}}
\newcommand{\eiso}{E_{iso}}
\newcommand{\epkz}{E_{p,z}}
\newcommand{\durz}{T_{90,z}}
\newcommand{\pbol}{P_{bol}}
\newcommand{\sbol}{S_{bol}}
\newcommand{\epk}{E_{p}}
\newcommand{\dur}{T_{90}}
\newcommand{\pph}{P_{50-300}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\label{sec:results}
    ...
    \subsection*{Average Side Chain coordinates as the Best Representation of Protein 3D Structure}
        ...
        Based on the observations described in the previous paragraphs, we keep only variables measured based on average side chain properties and coordinates throughout the rest of the analysis and omit all other possible measures that show only weaker correlations with other site-specific characteristics.
        \\


    \subsection*{Identifying Determinants of Sequence-Structure Correlation Strengths}

        In order to identify the potential contributing factors to the strength of sequence--structure correlations, we first employ one of the simplest nonparametric yet powerful tests of statistical dependence between variables in a collection of observations, that is, we construct the Spearman correlation matrix of all pdb-level structure and sequence properties. The choice of Spearman vs. the popular Pearson's correlation measure is to minimize the effects of any potential nonlinear relationships between the set of variables on the measures of associations between the variables.

        The exclusion of alternative measures of structural quantities as described in the previous section, results in a significant reduction in the number of pdb-level variables to be further analysed, from $$ to $$, without compromising generality and comprehensiveness of the analysis.

        Now with the reduced data set at hand, we attempt to identify the main determinants of each sequence-structure relation of interest using regularized regression methods (xx) on the original dataset of pdb-level properties. , also on its rank transformation to ensure non-linearities

        first remove all other sequence-structure correlations from the list of potential contributing factors. As explained in the previous sections, Then we apply regularized regression methods


        In the following lines we summarize the primary factors affecting the strength of individual correlations of interest.

        -- most importantly WCN, RSA $\Delta\Delta G$ Entropy, and Voronoi cell volume and surface area --
...
....
        ...
        \begin{figure}[tbh]
            \begin{center}
            \includegraphics[width=6.9in]{cordiv_similarities_r4sJC.pdf} % Fig. 1
            \end{center}
        ...
        \begin{figure}[tbh]
            \begin{center}
            \includegraphics[width=6.9in]{/best_wcn/selected_variables/boxplot_wcn_all_in_one.pdf} % Fig. 1
            \end{center}
        ...
        \begin{figure}[tbh]
            \begin{center}
            \includegraphics[width=6.9in]{/best_bf/selected_variables/boxplot_bf_all_in_one.pdf} % Fig. 1
            \end{center}
        ...
        \begin{figure}[tbh]
            \begin{center}
            \includegraphics[width=6.9in]{/best_vorvol/selected_variables/boxplot_vorvol_all_in_one.pdf} % Fig. 1
            \end{center}
