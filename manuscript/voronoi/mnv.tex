...
% Tables
    ...
    \begin{table}[htbp]
    \caption{Median best free parameter values of the Contact Number (CN) and the Weighted Contact Number with power-law kernel (WCN) that result in the strongest median Spearman's correlation ($\rho$) of CN \& WCN with three site-specific quantities (evolutionary rates (r4sJC), sequence entropy, and average side-chain B factor) in the entire dataset of 209 proteins. The $25\%$ \& $75\%$ quartile distances in the distribution of parameters from the median are given as superscripts and subscripts to the median values. \label{tab:best_params}}
    \bigskip
    \centerline{
    \begin{tabular}{|l|c|c|}
      \hline
      Correlation with  & $r_0[\AA]$ (CN)      & $\alpha$ (WCN)        \\ \hline
      r4sJC             & $14.3^{+5.3}_{-4.0}$ & $-2.3^{+0.8}_{-0.4}$  \\
      Seq. Entropy      & $12.4^{+5.5}_{-2.6}$ & $-2.2^{+0.8}_{-0.4}$  \\
      B factor          & $13.1^{+4.0}_{-2.2}$ & $-2.2^{+0.5}_{-0.4}$  \\
      \hline
    \end{tabular}}
    \end{table}
