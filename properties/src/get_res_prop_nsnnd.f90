! This Fortran program attempts to find the nearest neighbor distances (nsnnd, ns standing for Non-Sequential) of sites in proteins, EXCLUDING THE NEAREST THE SITES BEFORE AND AFTER THE GIVEN SITE OF INTEREST.
...
! Amir Shahmoradi, Thursday 8:31:00 PM, April 31 2015, iCMB, UT Austin

program get_res_prop_nsnnd
implicit none
! pdb file variables:
...
! input files and variables:
  ...
  character(len=300) :: nnd_out	                                           ! output file containing on each line, the nsnnd of each site.
...
! input files variables:
  ...
  integer            :: ios
! variables and parameters for nsnnd calculations:
  real*8 , dimension(:)     , allocatable :: nsnnd                    ! The array of nsnnd for all sites in a given protein.
...
! OPEN OUTPUT FILE:
  ...
  write(nnd_out_unit,'(7A20)') 'pdb','aa1_resname','aa2_resname','aa1_resnum','aa2_resnum','seq_distance','nsnnd'
...
do ii = 1,npdb
  ...
  allocate(crd(3,nres),bfactor(nres),nsnnd(nres),nnid(nres),res_name(nres),res_num(nres))
  ...
  ! NOW CALCULATE nsnnd FOR ALL VALUES OF PARAMETERS:
  call nsnnd_finder(nres,crd,nnid,nsnnd)
  
  ! NOW WRITE OUT THE FIRST FOUR LINES OF THE OUTPUT nsnnd FILE
  do j = 1,nres
    write(nnd_out_unit,'(3A20,3I20,1F20.3)') pdb,res_name(j),res_name(nnid(j)),res_num(j),res_num(nnid(j)),abs(res_num(j)-res_num(nnid(j))),nsnnd(j)
  end do

  deallocate(crd,bfactor,nsnnd,nnid,res_name,res_num)
...
end program get_res_prop_nsnnd


! This Fortran subroutine takes in the 3D coordinates of a set of atoms (or sites). On the output it gives the nearest neighbor distance (nsnnd) and the corresponding ID of the neighboring site for all the atoms that were given in the input.

subroutine nsnnd_finder(nres,crd,nnid,nsnnd)
  implicit none
  integer, intent(in)          :: nres
  real*8 , intent(in)          :: crd(3,nres)
  real*8 , intent(out)         :: nsnnd(nres)
