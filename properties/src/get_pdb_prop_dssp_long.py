...
# Amir Shahmoradi, 5:00 PM, Monday Aug 25 2014, Wilke Lab, iCMB, The University of Texas at Austin.
# ATTN: This code is exactly the same as get_pdb_prop_dssp.py with the only difference that it outputs data in LONG TABLE format.
...
def get_pdb_props(pdb_path,dssp_out):
    ...
    pdb_name = pdb_path.split('/')[-1][-10:-4] # split element 4 of the filecontent list (space-delimited). the 0th element is number of residues in pdb
    ...
    for record in fileContents[25:len(fileContents)]:
        ...
        if AA != ('!' or '*'):
            counter += 1
            if AA == ('X'):
                print 'potential Histidine variant HID, HIE or HIP found in pdb file ', pdb_name, 'residue number ', counter
...
Usage:'''
    ...
    if os.path.isfile(sum_out):
       ...
       sum_out_file.write('pdb' + '\t' + 'variable' + '\t' + 'value' + '\n')

    pdb_name = pdb_path.split('/')[-1][-10:-4] # split element 4 of the filecontent list (space-delimited). the 0th element is number of residues in pdb
    for key in pdb:
        sum_out_file.write( pdb_name + '\t' + key + '\t' + pdb[key] + '\n')
