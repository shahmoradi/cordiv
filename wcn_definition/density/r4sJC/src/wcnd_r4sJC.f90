! This Fortran program attempts to find the best performing cutoff distance in the original definition of Contact Number (CN) as used in my previous work "structural prediction of ER" that results in the highest Spearman correlation between evolutionary rates (r4sJC) of residues in pdb files (Echave pdb data set) and CN. The results are then compared to the correlations based on the original definition of WCN (inverse-square polwer law).
! GOALS:
...
! Amir Shahmoradi, Monday 3:56 AM, March 2 2015, iCMB, UT Austin
...
! variables and parameters for wcn calculations and correlations:
  ...
  real*8          , parameter                   :: cutoff_distance_stride = 0.1 
...
! First get the command line arguments
  ...
  if (command_argument_count()/=4) then
    ...
    write(*,*) "Correct use:"
    write(*,'(1A140)') "./a.out <input: pdb file> <input: Eleisha's Seq.Ent file> <output: pdb cutoff_distance file> <output summary file: best performing cutoff_distances>"
    write(*,*)
    stop
  end if
...
! NOW READ IN THE SEQUENCE ENTROPY FROM ELEISHA'S INPUT FILE:
  ...
  ios = 0; ii = 0
  do while(ios==0)
    read(elj_in_unit,*,iostat=ios) i, elj_pdb_name, elj_chain_id, elj_res_num, jec_r4sJC, elj_ddgent
    ...
    elseif (ios>0) then
      ...
      if (pdb_name(1:4) == elj_pdb_name) then
        ...
        pdb_seqent(ii) = jec_r4sJC
        pdb_ddgent(ii) = elj_ddgent
        !write(*,'(1I10,2E15.5)') ii,jec_r4sJC,pdb_seqent(ii)
      end if
      if (ii>nres) then
...
! NOW CALCULATE WCN FOR EACH CA ATOM, FOR DIFFERENT cutoff_distance VALUES IN THE DEFINITION OF WCN AND MEASURE THE CORRESPONDING SPEARMAN CORRELAITONS OF WCN AND PDB_SEQENT.
