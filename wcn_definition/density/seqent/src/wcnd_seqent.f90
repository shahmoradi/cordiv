! This Fortran program attempts to find the best performing cutoff distance in the original definition of Contact Number (CN) as used in my previous work "structural prediction of ER" that results in the highest Spearman correlation between sequence netropies of residues in pdb files (Echave pdb data set) and CN. The results are then compared to the correlations based on the original definition of WCN (inverse-square polwer law).
! GOALS:
...
! Amir Shahmoradi, Friday 12:36 AM, July 18 2014, iCMB, UT Austin


program wcnd_ent_cor
implicit none
! pdb file variables:
