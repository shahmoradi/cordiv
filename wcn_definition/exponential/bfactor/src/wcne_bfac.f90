! This Fortran program attempts to find the best performing exponential definition (as in Hinsen et al. 2000) of the variable Weighted Contact Number (wcn) that results in the highest Spearman correlation between the B-factors of CA atoms in pdb files (on Echave pdb data set) and wcn. The results are then compared to the correlations based on the original definition of WCN (inverse-square polwer law).
! GOALS:
!       -- To see if the exponential definition of wcn outperforms the original definition WCN.
!       -- If the free parameter of the exponential definition (the expmean variable) is structure dependent or approximately the same for all PDB structures.
! INPUT:
!       -- Path to the input pdb file
!       -- Path and name of the output file containing Spearman correlations of wcn-bfac for each pdb structure on each line.
!       -- Path and name of the output summary file containing the name of the pdbs on the first column, the best performing expmean, the corresponding Spearman corr., the corresponding Spearman corr. for the default power-law definition of WCN, the difference between absolute values of the two correlations and the length (nres) of the protein, each on a separate column.
! METHOD:
!       The code searches for the best performing expmean for WCN in the stupidest and slowest, but simplest way possible, which does not matter, thanks for Fortran's miraculous runtime speed. It searches for the best value of the free parameter expmean.

! Amir Shahmoradi, Monday 8:04 PM, July 14 2014, iCMB, UT Austin
...
! input files:
  ...
  character(len=300) :: exp_out	                                         ! output file containing on each line, the Spearman correlations of Seq.Ent of a single pdb file with different exponential definitions of WCN (different expmeans).
  character(len=300) :: sum_out	                                         ! output summary file containing the pdb names and the best performing expmeans and the corresponding Spearman correlations and the length of the protein on the last column.
...
! variables and parameters for wcn calculations and correlations:
  real*8 , parameter                      :: expmean_min = 0.0d0 
  real*8 , parameter                      :: expmean_max = 50.d0 
  real*8 , parameter                      :: expmean_stride = 0.2
...
! First get the command line arguments
  ...
  if (command_argument_count()/=3) then
    ...
    write(*,*) "Correct use:"
    write(*,'(1A140)') "./a.out <input: pdb file> <output: pdb EXPMEAN file> <output summary file: best performing EXPMEANs>"
    write(*,*)
    stop
  end if
...
! NOW CALCULATE WCN FOR EACH CA ATOM, FOR DIFFERENT EXPMEAN VALUES IN THE exponential DEFINITION OF WCN AND MEASURE THE CORRESPONDING SPEARMAN CORRELAITONS OF WCN AND PDB_SEQENT.

  allocate(wcn(nres))
  call get_wcn_default(nres,CAcrd,wcn)
  sp_default = spear(nres,wcn(1:nres),CAbfac(1:nres))
  do i = 1,nexpmean
    expmean(i) = expmean_min + dble(i)*expmean_stride
    call get_wcn_exponential(nres,CAcrd,expmean(i),wcn)
    !write(*,*) nres; read(*,*)
...
! NOW WRITE OUT THE FIRST FOUR LINES OF THE OUTPUT WCN FILE
  ! open output files:
    ...
    if (exp_out_exists) then
      ...
      write(exp_out_unit,exp_output_format) 'EXPMEANs',(expmean(i),i=1,nexpmean)
    end if

    write(exp_out_unit,exp_output_format) pdb_name,(sp_cor(i),i=1,nexpmean)
    ...
    if (sum_out_exists) then
      ...
      write(sum_out_unit,'(6A20)') 'pdb_name','expmean_best','sp_best','sp_default','sp_diff','nres'
    end if
  
  
  abs_sp_cor = abs(sp_cor)
  write(sum_out_unit,'(1A20,4F20.3,1I20)') pdb_name,expmean(maxloc(abs_sp_cor)),sp_cor(maxloc(abs_sp_cor)),sp_default,abs_sp_cor(maxloc(abs_sp_cor))-abs(sp_default),nres
...
! This Fortran subroutine takes in the 3D coordinates of a set of atoms (presumably C-alpha of pdb files). On the output it gives the WCN of all the atoms that were given in the input.
! Amir Shahmoradi, Wednesday 3:38 PM, June 25 2014, iCMB, UT Austin

subroutine get_wcn_exponential(natoms,crd,expmean,wcn)
implicit none
integer, intent(in) :: natoms
real*8, intent(in) :: expmean,crd(natoms,3)
real*8, intent(out) :: wcn(natoms)
integer :: i,j
real*8 :: distance
...
do i=1,natoms
  do j=1,natoms
    if (j/=i) then
      ...
      wcn(i) = wcn(i) + exp(-distance/expmean)
    end if
  end do
end do
!write(*,*) wcn; read(*,*)
end subroutine get_wcn_exponential
