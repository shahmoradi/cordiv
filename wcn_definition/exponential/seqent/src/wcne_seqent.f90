! This Fortran program attempts to find the best performing exponential definition (as in Hinsen et al. 2000) of the variable Weighted Contact Number (wcn) that results in the highest Spearman correlation between the sequence entorpies of pdb files in Echave data set (calculated by Eleisha) and wcn. The results are then compared to the correlations based on the original definition of WCN (inverse-square polwer law).

! GOALS:
!       -- To see if the exponential definition of wcn outperforms the original definition WCN.
!       -- If the free parameter of the exponential definition (the STDEV variable) is structure dependent or approximately the same for all PDB structures.
! INPUT:
!       -- Path to the input pdb file
!       -- Path and name of the output file containing Spearman correlations of wcn-bfac for each pdb structure on each line.
!       -- Path and name of the output summary file containing the name of the pdbs on the first column, the best performing stdev, the corresponding Spearman corr., the corresponding Spearman corr. for the default power-law definition of WCN, the difference between absolute values of the two correlations and the length (nres) of the protein, each on a separate column.
! METHOD:
!       The code searches for the best performing stdev for WCN in the stupidest and slowest, but simplest way possible, which does not matter, thanks for Fortran's miraculous runtime speed. It searches for the best value of the free parameter stdev.

! Amir Shahmoradi, Thursday 6:37 PM, July 17 2014, iCMB, UT Austin
...
! input files:                                                                                      
  ...
  character(len=300)                            :: exp_out                                          ! output file containing on each line, the Spearman correlations of Seq.Ent of a single pdb file with different definitions of WCN (different STDEVs).
  character(len=300)                            :: sum_out                                          ! output summary file containing the pdb names and the best performing STDEVs and the corresponding Spearman correlations and the length of the protein on the last column.
...
! First get the command line arguments
  ...
  if (command_argument_count()/=4) then
    ...
    write(*,*) "Correct use:"
    write(*,'(1A140)') "./a.out <input: pdb file> <input: Eleisha's Seq.Ent file> <output: pdb expmean file> <output summary file: best performing expmeans>"
    write(*,*)
    stop
  end if
...
! NOW CALCULATE WCN FOR EACH CA ATOM, FOR DIFFERENT expmean VALUES IN THE DEFINITION OF WCN AND MEASURE THE CORRESPONDING SPEARMAN CORRELAITONS OF WCN AND PDB_SEQENT.
...
! This Fortran subroutine takes in the 3D coordinates of a set of atoms (presumably C-alpha of pdb files).
! Amir Shahmoradi, Wednesday 3:38 PM, June 25 2014, iCMB, UT Austin
