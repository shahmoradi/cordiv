...
for (i in 1:length(varnames_short))
{
  ...
  #temp$vedge   = 10.^temp$vedge
  temp$varea   = 10.^temp$varea
  temp$vvolume = 10.^temp$vvolume
  list_temp[[i]] = temp
  ...
  #temp_closed$vedge   = 10.^temp_closed$vedge
  temp_closed$varea   = 10.^temp_closed$varea
  temp_closed$vvolume = 10.^temp_closed$vvolume
  ...
  #temp_open$vedge   = 10.^temp_open$vedge
  temp_open$varea   = 10.^temp_open$varea
  temp_open$vvolume = 10.^temp_open$vvolume
...
for (i in 1:length(varnames_short))
{
  ...
  filename = paste0('../figures/adjacent_averaging_screen/voro_nonlog/',varnames_short[i],'.pdf')
