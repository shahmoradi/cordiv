...
write.csv( wcn_scors_all_pdbs, "../tables/best_wcn/select_variables/wcn_scors_all_pdbs.csv", row.names=F )
...
for (wcn in levels(wcn_scors_all_pdbs$wcn))
{
  ...
  filename = paste0("../tables/best_wcn/select_variables/",wcn,'_scors_summary.csv')
...
{
  filename = paste0("../tables/best_wcn/select_variables/",wcn_list[[i]][1],'_scors_summary.csv')
  wcn_scors_summary1 = read.csv( filename, header = T )
  for (j in (i+1):length(wcn_list))
  {
    filename = paste0("../tables/best_wcn/select_variables/",wcn_list[[j]][1],'_scors_summary.csv')
    ...
    filename = paste0('../tables/best_wcn/select_variables/diff_',wcn_list[[i]][1],'_',wcn_list[[j]][1],'.csv')
    cat (filename, '\n')
...
for (i in 1:(length(wcn_list)-1))
{
  ...
  for (j in (i+1):length(wcn_list))
  {
    filename = paste0("../tables/best_wcn/select_variables/",wcn_list[[j]][1],'_scors_summary.csv')
    wcn_scors_summary2 = read.csv( filename, header = T )
    filename = paste0('../figures/best_wcn/select_variables/abs(',wcn_list[[i]][1],'_',wcn_list[[j]][1],').pdf')
    cat (filename, '\n')
...
{ 
  ...
  filename = paste0('../figures/best_wcn/select_variables/boxplot_',variable,'.pdf')
...
wcn_list = c('wcnSC','wcnAA','wcnCB','wcnCA','wcnN','wcnC','wcnO')
counter = 0
filename = paste0('../figures/best_wcn/select_variables/boxplot_wcn_all_in_one.pdf')
...
{ 
  ...
  temp_data_variable = reshape(temp_data_variable_long, timevar = 'wcn', idvar = 'pdb', direction = 'wide')
  temp_data_variable = subset (temp_data_variable, select = -c(pdb))
  colnames(temp_data_variable) = substr(levels(wcn_scors_all_pdbs$wcn),start=4,stop=5)  # remove 'wcn' from the names of the variables. It is unnecessary and ugly in the figures. Only the the atom names will remain.
...
wcn_list = c('wcnSC','wcnAA','wcnCB','wcnCA','wcnN','wcnC','wcnO')
counter = 0
filename = paste0('../figures/best_wcn/select_variables/boxplot_wcn_two_in_one.pdf')
