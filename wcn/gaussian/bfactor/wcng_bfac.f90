...
! Amir Shahmoradi, Monday 4:14 PM, July 14 2014, iCMB, UT Austin
...
! input files:
  ...
  character(len=300) :: exp_out	                                         ! output file containing on each line, the Spearman correlations of Seq.Ent of a single pdb file with different Gaussian definitions of WCN (different STDEVs).
...
! First get the command line arguments
  ...
  if (command_argument_count()/=3) then
    ...
    write(*,*) "Correct use:"
    write(*,'(1A140)') "./a.out <input: pdb file> <output: pdb STDEV file> <output summary file: best performing STDEVs>"
    write(*,*)
    stop
  end if
...
! NOW CALCULATE WCN FOR EACH CA ATOM, FOR DIFFERENT STDEV VALUES IN THE GAUSSIAN DEFINITION OF WCN AND MEASURE THE CORRESPONDING SPEARMAN CORRELAITONS OF WCN AND PDB_SEQENT.
...
! NOW WRITE OUT THE FIRST FOUR LINES OF THE OUTPUT WCN FILE
  ! open output files:
    ...
    if (sum_out_exists) then
      ...
      write(sum_out_unit,'(6A20)') 'pdb_name','std_best','sp_best','sp_default','sp_diff','nres'
    end if
