! This Fortran program attemots to find the exponent of the definition of the variable Weighted Contact Number (wcn) (also weighted by the expected distance of the two sites) that results in the highest Spearman correlation between the sequence entropies of Eleisha Jackson (on Echave pdb data set) and wcn.
! GOALS:
...
! Amir Shahmoradi, Friday 12:44 PM, June 27 2014, iCMB, UT Austin
...
! variables and parameters for wcn calculations and correlations:
  real*8, parameter  :: alpha_min = -10.d0                               ! alpha stands for the exponent of inter-residue distance in the definition of WCN. alpha = -2 corresponds to the original definition of WCN.
  real*8, parameter  :: alpha_max =  10.d0                               ! alpha stands for the exponent of inter-residue distance in the definition of WCN. alpha = -2 corresponds to the original definition of WCN.
  ...
  real*8, parameter  :: alpha_default = 0.d0                            ! The default value of alpha as in the original definition of wcn.
  ...
  real*8, dimension(:,:)   , allocatable :: CAcrd                        ! this vector will contain the coordinates of the c-alpha carbons in the pdb file. dimension is (nres,3). 
  real*8, dimension(:)     , allocatable :: CAresnum                     ! this vector will contain the site number of the c-alpha carbons in the sequence of the protein. dimension is (nres). 
...
! NOW RECORD THE CA COORDINATES OF THE PDB FILE:
  ...
  open(unit=pdb_in_unit,file=trim(adjustl(pdb_in)),status='old')
  allocate(CAcrd(nres,3),CAresnum(nres))
    ...
    elseif (trim(adjustl(record(i)))=='ATOM') then
      ...
      elseif (trim(adjustl(atom_name(i)))=='CA') then
        ...
        elseif (res_num_old == res_num(i) .and. res_cod_old == res_code(i)) then
          write(*,*) 'Duplicate Amino Acid found in structure!', res_num(i), res_name(i)
          i = i + 1
          cycle
        else
          ii = ii + 1
...
! NOW CALCULATE WCN FOR EACH CA ATOM, FOR DIFFERENT EXPONENT (ALPHA) VALUES IN THE DEFINITION OF WCN AND MEASURE THE CORRESPONDING SPEARMAN CORRELAITONS OF WCN AND PDB_SEQENT.
  ...
  allocate(wcn(nres))
  call get_wcn(nres,CAcrd,CAresnum,alpha_default,wcn)
  sp_default = spear(nres,wcn(1:nres),pdb_seqent(1:nres))
  do i = 1,nalpha
    alpha(i) = alpha_min + dble(i)*alpha_stride
    call get_wcn(nres,CAcrd,CAresnum,alpha(i),wcn)
    !write(*,*) nres; read(*,*)
...
subroutine get_wcn(natoms,crd,site,alpha,wcn)
implicit none
integer, intent(in) :: natoms
!real*8 :: alpha_temp
real*8, intent(in) :: alpha,crd(natoms,3),site(natoms)
real*8, intent(out) :: wcn(natoms)
integer :: i,j
real*8 :: distance_sq,nonlocality

wcn = 0.d0

if (abs(alpha) < 0.001d0) then
  write(*,*) 'calculating default case'
end if
  ...
  do j=1,natoms
    if (j/=i) then
      if (abs(alpha) < 0.001d0) then
        ...
        nonlocality = (abs(site(i)-site(j)))**alpha
      end if
      distance_sq = (crd(i,1)-crd(j,1))**2 + (crd(i,2)-crd(j,2))**2 + (crd(i,3)-crd(j,3))**2
      wcn(i) = wcn(i) + nonlocality/distance_sq
    end if
  end do
end do
!write(*,*) wcn; read(*,*)
