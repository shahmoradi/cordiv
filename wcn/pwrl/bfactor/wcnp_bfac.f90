! This Fortran program attempts to find the exponent of the definition of the variable Weighted Contact Number (wcn) that results in the highest Spearman correlation between the B-factors of CA atoms in pdb files (on Echave pdb data set) and wcn.
! GOALS:
!       -- To see if the best performing wcn exponent is pdb structure dependent, or is approximately the same for all pdbs.
...
!       -- Path and name of the output summary file containing the name of the pdbs on the first column, the best performing exponent, the corresponding Spearman corr., the corresponding Spearman corr. for the default exponent of alpha = -2, the difference between absolute values of the two correlations and the length (nres) of the protein, each on a separate column.
! METHOD:
...
! Amir Shahmoradi, Monday 12:27 PM, July 14 2014, iCMB, UT Austin
...
! First get the command line arguments
  ...
  if (command_argument_count()/=3) then
    ...
    write(*,*) "Correct use:"
    write(*,'(1A140)') "./a.out <input: pdb file> <output: pdb exponent file> <output summary file: best performing exponents>"
    write(*,*)
    stop
  end if
