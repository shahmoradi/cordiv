! This Fortran program attempts to find the best performing Gaussian definition (as in Hinsen et al. 2000) of the variable Weighted Contact Number (wcn) that results in the highest Spearman correlation between the sequence entorpies of pdb files in Echave data set (calculated by Eleisha) and wcn. The results are then compared to the correlations based on the original definition of WCN (inverse-square polwer law).

! GOALS:
!       -- To see if the Gaussian definition of wcn outperforms the original definition WCN.
!       -- If the free parameter of the Gaussian definition (the STDEV variable) is structure dependent or approximately the same for all PDB structures.
! INPUT:
...
! Amir Shahmoradi, Monday 5:32 PM, July 14 2014, iCMB, UT Austin
...
! First get the command line arguments
  ...
  if (command_argument_count()/=4) then
    ...
    write(*,*) "Correct use:"
    write(*,'(1A140)') "./a.out <input: pdb file> <input: Eleisha's Seq.Ent file> <output: pdb STDEV file> <output summary file: best performing STDEVs>"
    write(*,*)
    stop
  end if
...
! NOW CALCULATE WCN FOR EACH CA ATOM, FOR DIFFERENT STDEV VALUES IN THE DEFINITION OF WCN AND MEASURE THE CORRESPONDING SPEARMAN CORRELAITONS OF WCN AND PDB_SEQENT.

  allocate(wcn(nres))
  call get_wcn_default(nres,CAcrd,wcn)
  sp_default = spear(nres,wcn(1:nres),pdb_seqent(1:nres))
  do i = 1,nstdev
    stdev(i) = stdev_min + dble(i)*stdev_stride
    call get_wcn_gaussian(nres,CAcrd,stdev(i),wcn)
    !write(*,*) nres; read(*,*)
...
! NOW WRITE OUT THE FIRST FOUR LINES OF THE OUTPUT WCN FILE
  ! open output files:
    ...
    if (exp_out_exists) then
      ...
      write(exp_out_unit,exp_output_format) 'STDEVs',(stdev(i),i=1,nstdev)
    end if

    write(exp_out_unit,exp_output_format) pdb_name,(sp_cor(i),i=1,nstdev)
  ...
  write(sum_out_unit,'(1A20,4F20.3,1I20)') pdb_name,stdev(maxloc(abs_sp_cor)),sp_cor(maxloc(abs_sp_cor)),sp_default,abs_sp_cor(maxloc(abs_sp_cor))-abs(sp_default),nres
...
subroutine get_wcn_gaussian(natoms,crd,stdev,wcn)
implicit none
integer, intent(in) :: natoms
real*8, intent(in) :: stdev,crd(natoms,3)
real*8, intent(out) :: wcn(natoms)
integer :: i,j
real*8 :: distance_sq,variance
...
do i=1,natoms
  do j=1,natoms
    if (j/=i) then
      distance_sq = (crd(i,1)-crd(j,1))**2 + (crd(i,2)-crd(j,2))**2 + (crd(i,3)-crd(j,3))**2
      wcn(i) = wcn(i) + exp(-distance_sq/variance)
    end if
  end do
end do
!write(*,*) wcn; read(*,*)
end subroutine get_wcn_gaussian
