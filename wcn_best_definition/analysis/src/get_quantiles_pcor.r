# This R script generates the quantile tables and the corresponding plots for the free parameters of different models of WCN using data that controls for the distance of sites from the center of proteins.
# Amir Shahmoradi, Monday 4:44 PM, April 20, 2015, Wilke Lab, iCMB, UT Austin
...
for (dataframe in dflist)
{
  ...
  write.csv( quantiles, file = paste0('../tables/get_quantiles_pcor/sp_raw/',names(dflist)[[counter]],'_quantiles.csv'), row.names=F)
  ...
  pdf( paste0("../figures/get_quantiles_pcor/sp_raw/spcor_",names(dflist)[[counter]],".pdf"), width=4.5, height=4, useDingbats=FALSE )
  ...
  rownames(quantiles) = NULL
  write.csv( quantiles, file = paste0('../tables/get_quantiles_pcor/sp_abs/',names(dflist)[[counter]],'_quantiles.csv'), row.names=F)
  ...
  pdf( paste0("../figures/get_quantiles_pcor/sp_abs/spcor_",names(dflist)[[counter]],".pdf"), width=4.5, height=4, useDingbats=FALSE )
  #par( mai=c(0.65, 0.65, 0.1, 0.05), mgp=c(2, 0.5, 0), tck=-0.03 )
  ...
  pdf( paste0("../figures/get_quantiles_pcor/stdev_",names(dflist)[[counter]],".pdf"), width=4.5, height=4, useDingbats=FALSE )
  #par( mai=c(0.65, 0.65, 0.1, 0.05), mgp=c(2, 0.5, 0), tck=-0.03 )
...
write.csv( best_params, file = paste0('../tables/get_quantiles_pcor/best_params_partial.csv'), row.names=F)
...
for (dataframe in dflist)
{
  ...
  #Plot Average Absolute Spearman correlation vs. value of free parameter
  pdf( paste0("../figures/get_quantiles_pcor/screen_plots/spcor_",names(dflist)[[counter]],".pdf"), width=4.5, height=4, useDingbats=FALSE )
  par( mai=c(0.65, 0.65, 0.1, 0.05), mgp=c(2, 0.5, 0), tck=-0.03 )
