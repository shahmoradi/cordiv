...
! input files and variables:
  ...
  character(len=300) :: seq_in                                             ! input file containing sequence variability data for all pdb files in the format as in "seq_var_data.in".
...
! input files variables:
  ...
  integer            :: seq_in_unit=12                                      ! input file units
  integer            :: exp_out_unit=21, sum_out_unit=22                    ! output file units
  integer            :: ios
  character(len=100) :: exp_output_format
! sequence input file variables:
  character(len=6)   :: seq_pdb_name
  !character(len=1)   :: elj_chain_id
  !integer            :: elj_res_num
  real*8, dimension(:) , allocatable :: r4sJC, seqent, ddgent  
...
! First get the command line arguments
  ...
  call get_command_argument(4,seq_in)
...
! OPEN INPUT FILES:
  ...
  open(unit=seq_in_unit,file=trim(adjustl(seq_in)),status='old')
  read(co_in_unit,*)        ! read file header
  read(crd_in_unit,*)       ! read file header
  read(seq_in_unit,*)       ! read file header
! OPEN OUTPUT FILES:
...
do ii = 1,npdb
  ...
  allocate(r4sJC(nres),seqent(nres),ddgent(nres))
  ! Now read the crd file:
  do j = 1,nres
    read(crd_in_unit,*) pdb_name, res_name, res_num, crd(1:3,j), bfactor(j)
    read(seq_in_unit,*) seq_pdb_name, r4sJC(j), seqent(j), ddgent(j)
    if (pdb /= pdb_name .or. pdb /= seq_pdb_name) then
      write(*,*) 'Something is heavily fishy here!'
      write(*,*) 'pdb names do not match!', pdb, pdb_name, seq_pdb_name; write(*,*) 'Program Aborted'
      STOP
    end if
  end do
