...
!       -- Path to the input Voronoi Volumes file (../../../properties/res_prop_voronoiSC.out)
!       -- Path and name of the output file containing Spearman correlations of CN-vorvol for each pdb structure on each line.
...
! Amir Shahmoradi, Wednesday 10:51 AM, January 13 2015, ICES, UT Austin

program wcn_vorvol_cor
implicit none
! pdb file variables:
...
! input files and variables:
  ...
  character(len=300) :: vor_in                                             ! input file containing Voronoi data for all pdb files in the format as in "seq_var_data.in".
...
! input files variables:
  ...
  integer            :: vor_pdb_name_unit=12                                      ! input file units
  integer            :: exp_out_unit=21, sum_out_unit=22                    ! output file units
  integer            :: ios
  character(len=100) :: exp_output_format
! Voronoi input file variables:
  character(len=6)   :: vor_pdb_name
  integer            :: sizeSC,sizeAA,nvertices,nedges,nfaces
  real*8             :: resvol,edge_length_total,area
...
! First get the command line arguments
  ...
  call get_command_argument(4,vor_in)
...
! OPEN INPUT FILES:
  ...
  open(unit=vor_pdb_name_unit,file=trim(adjustl(vor_in)),status='old')
  read(co_in_unit,*)        ! read file header
  read(crd_in_unit,*)       ! read file header
  read(vor_pdb_name_unit,*)       ! read file header
! OPEN OUTPUT FILES:
...
do ii = 1,npdb
  ...
  allocate(volume(nres))
  ! Now read the crd file:
  do j = 1,nres
    read(crd_in_unit,*) pdb_name, res_name, res_num, crd(1:3,j), bfactor(j)
    read(vor_pdb_name_unit,*) vor_pdb_name,res_name,res_num,sizeSC,sizeAA,resvol,nvertices,nedges,edge_length_total,nfaces,area,volume(j)
    if (pdb /= pdb_name .or. pdb /= vor_pdb_name) then
      write(*,*) 'Something is heavily fishy here!'
      write(*,*) 'pdb names do not match!', pdb, pdb_name, vor_pdb_name; write(*,*) 'Program Aborted'
      STOP
    end if
  end do
  ...
  do i = 1,nstride
    ...
    sp_cor(i) = spear(nres,wcn,volume)
...
close(co_in_unit)
close(crd_in_unit)
close(vor_pdb_name_unit)
close(exp_out_unit)
close(sum_out_unit)

end program wcn_vorvol_cor
