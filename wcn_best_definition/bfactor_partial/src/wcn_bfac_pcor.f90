...
! First get the command line arguments
  ...
  call get_command_argument(4,distance_in)
...
do ii = 1,npdb
  ...
  allocate(distance_from_COM(nres))
  ! Now read the crd file:
  do j = 1,nres
    read(crd_in_unit,*) pdb_name, res_name, res_num, crd(1:3,j), bfactor(j)
    read(distance_in_unit,*) distance_pdb_name, res_name, res_num, distance_from_COM(j)
    if (pdb /= pdb_name .or. pdb /= distance_pdb_name) then
      write(*,*) 'Something is heavily fishy here!'
      write(*,*) 'pdb names do not match!', pdb, pdb_name, distance_pdb_name; write(*,*) 'Program Aborted'
      STOP
    end if
  end do
